 ...
 * объекта, а это то, для чего собственно и разрабатывался данный фреймворк, то на него накладываются ограничения, описанные в следующем разделе. 
 *
 * API и асинхронные методы
 * ----------------
 * Рассматриваемые в данном разделе ограничения, следует воспринимать как требования, если вы хотите использовать реализованную во фреймворке 
 * JSON-RPC инфраструктуру. Или как набор рекомендаций, если вы захотите реализовать свой протокол взаимодействия, без потери совместимости.
 * - Все входящие параметры должны быть объедены в одну структуру(struct). Т.е. на входе метода всегда один параметр типа struct, в которой
 * описываются все входящие переменные, даже если переменная одна или они отсутствуют (пустая структура). Это позволяет:
 *    - Добавлять или удалять параметры вызова без необходимости внесение изменения в интерфейс и реализацию
 *    - Добавлять параметры и реализовать логику без необходимости внесения изменения в декларации протокола взаимодействия (например JSON-RPC). 
 *      Т.е. сначала  реализовали, убедились что работает со значением по умолчанию, потом вносим изменения в протокола
 *    - Добавлять параметры в структуру и декларацию протокола взаимодействия (например JSON-RPC) без реализации (игнорируем). 
 *      Т.е. объявляем клиенту, что появились новые параметры, убеждаемся что он правильно их использует, потом реализуем соответствующую логику.  
 *    - Кроме того это требование JSON-RPC инфраструктуры WFC (на самом деле нет - эти ограничения можно обойти, но я вам не скажу как, так как 
 *      это не имеет особого смысла и требует написания большего объема кода) 
 ...
 *    - Однако это не означает, что реализация метода не может быть синхронный (вызов callback-функции сразу же после обработки запроса )  
 *    
 * 
 * Опции
 * ----------------
