...
#include <wfc/fas/scatter_hierarchy.hpp>
#include <wfc/fas/type_list.hpp>

namespace wfc{

template< typename TL = empty_type>
struct aspect;
...
#include <wfc/fas/detail/aspect.hpp>
...
template< typename T >
struct aspect
  : scatter_hierarchy< typename advice_list_traits<T>::type >
{
  ...
  struct select
  {
    typedef typename ::wfc::select<Tg, advice_list>::type type;
  };

  template<typename TT>
  struct has
  {
		enum { result = type_count<TT, advice_list>::result };
  };
...
private:
  ...
  F _aspect_for_each(H<empty_type>&, F f) { return f;}

  template< template<typename> class H, typename F >
  F _aspect_for_each(const H<empty_type>&, F f) const { return f;}
};
...
private:
  ...
  typedef typename merge< left_common_list,
...
struct advice_cast
{
  // typedef typename left_cast<T, typename A::aspect::advice_list>::type::advice_class type;
  typedef typename left_cast<T, typename A::advice_list>::type advice;
...
#include <wfc/fas/specialization/aspect.hpp>
